/*=============================================================================|
|  PROJECT SnapModbus                                                    1.0.1 |
|==============================================================================|
|  Copyright (C) 2023, 2025 Davide Nardella                                    |
|  All rights reserved.                                                        |
|==============================================================================|
|  SnapModbus is free software: you can redistribute it and/or modify          |
|  it under the terms of the Lesser GNU General Public License as published by |
|  the Free Software Foundation, either version 3 of the License, or           |
|  (at your option) any later version.                                         |
|                                                                              |
|  It means that you can distribute your commercial software linked with       |
|  SnapModbus without the requirement to distribute the source code of your    |
|  application and without the requirement that your application be itself     |
|  distributed under LGPL.                                                     |
|                                                                              |
|  SnapModbus is distributed in the hope that it will be useful,               |
|  but WITHOUT ANY WARRANTY; without even the implied warranty of              |
|  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the               |
|  Lesser GNU General Public License for more details.                         |
|                                                                              |
|  You should have received a copy of the GNU General Public License and a     |
|  copy of Lesser GNU General Public License along with Snap7.                 |
|  If not, see  http://www.gnu.org/licenses/                                   |
|=============================================================================*/
#include "snap_sysutils.h"
#include "mb_utils.h"


//-----------------------------------------------------------------------------
void CopyAndSwapReg16Pack(PRegisters16 Dst, PRegisters16 Src, int Amount)
{
	for (int c = 0; c < Amount; c++)
		Dst->WordArray[c] = SwapWord(Src->WordArray[c]);
}

//-----------------------------------------------------------------------------
void SwapReg16Pack(PRegisters16 RegPack, int Amount)
{
	for (int c = 0; c < Amount; c++)
		RegPack->WordArray[c] = SwapWord(RegPack->WordArray[c]);
}

//-----------------------------------------------------------------------------
void SwapReg32Pack(PRegisters32 RegPack, int Amount)
{
	for (int c = 0; c < Amount; c++)
		RegPack->DWordArray[c] = SwapDWord(RegPack->DWordArray[c]);
}

//-----------------------------------------------------------------------------
uint64_t SwapQWord(uint64_t Value) {
	uint64_t x = Value;
	x = (x & 0x00000000FFFFFFFF) << 32 | (x & 0xFFFFFFFF00000000) >> 32;
	x = (x & 0x0000FFFF0000FFFF) << 16 | (x & 0xFFFF0000FFFF0000) >> 16;
	x = (x & 0x00FF00FF00FF00FF) << 8 | (x & 0xFF00FF00FF00FF00) >> 8;
	return x;
}

//-----------------------------------------------------------------------------
void SwapReg64Pack(PRegisters64 RegPack, int Amount)
{
	for (int c = 0; c < Amount; c++)
		RegPack->QWordArray[c] = SwapQWord(RegPack->QWordArray[c]);

}

//-----------------------------------------------------------------------------
int RoundToNextByte(int Amount)
{
	int TotalByteSize = Amount / 8;
	if (Amount % 8 != 0)
		TotalByteSize++;
	return TotalByteSize;
}

//-----------------------------------------------------------------------------
void UnpackBits(uint8_t* Array, uint8_t* Pack, int Amount)
{
	uint8_t mask[8] = { 0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80 };
	int y = 0;

	for (int c = 0; c < Amount; c++)
	{
		Array[c] = (* Pack & mask[y++]) != 0;

		if (y == 8) {
			y = 0;
			Pack++;
		}
	}
}

//-----------------------------------------------------------------------------
void PackBits(uint8_t* Pack, uint8_t* Array, int Amount)
{
	uint8_t mask[8] = { 0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80 };
	int y = 0;

	for (int c = 0; c < Amount; c++)
	{
		if (y == 0)
			*Pack = 0;

		if (Array[c])
			*Pack |= mask[y];
		y++;
		if (y == 8) {
			Pack++;
			y = 0;
		}
	}
}

//-----------------------------------------------------------------------------
void CalcTimeout(longword Elapsed, PDeviceParams Params)
{
	longword Time = DeltaTime(Elapsed);
	if (Params->TimeMin > Time) Params->TimeMin = Time;
	if (Params->TimeMax < Time) Params->TimeMax = Time;
	Params->AutoTimeCalc_ms = (Params->TimeMin + Params->TimeMax); // The double of mean time i.e. ((min + max)/2) * 2
	if (Params->AutoTimeCalc_ms < Params->AutoTimeLimit_Min) Params->AutoTimeCalc_ms = Params->AutoTimeLimit_Min;
}
//-----------------------------------------------------------------------------
void ClearTimeout(PDeviceParams Params)
{
	Params->AutoTimeCalc_ms = Params->FixedTimeout_ms;
}
//-----------------------------------------------------------------------------
void SetDeviceDefaults(TDeviceParams& Params)
{
	Params.SerialFormat = def_SerialFormat; // RTU or ASCII
	Params.AutoTimeout = def_AutoTimeout;
	Params.AutoTimeLimit_Min = def_AutoTimeLimit_Min;
	Params.FixedTimeout_ms = def_FixedTimeout_ms;
	Params.AcceptBroadcast = def_AcceptBroadcast;
	// temporary values used to calc the dynamic timeout
	Params.TimeMin = def_FixedTimeout_ms;
	Params.TimeMax = 0;
	Params.AutoTimeCalc_ms = def_FixedTimeout_ms;
}
//-----------------------------------------------------------------------------
/* Table of CRC values for high-order byte */
static const uint8_t table_crc_hi[] = {
	
};

//-----------------------------------------------------------------------------
/* Table of CRC values for low-order byte */
static const uint8_t table_crc_lo[] = {
	
};
//------------------------------------------------------------------------------
// CRC 16 Calculation, adapted from:
// 
// MODBUS over Serial Line Specification and Implementation Guide V1.02
// Pag.42
//
//------------------------------------------------------------------------------
uint16_t CalcCRC(uint8_t* Buf, uint16_t Length)
{
	uint8_t crc_hi = 0xFF; /* high CRC byte initialized */
	uint8_t crc_lo = 0xFF; /* low CRC byte initialized */
	unsigned int i; /* will index into CRC lookup */

	/* pass through message buffer */
	while (Length--)
	{
		i = crc_lo ^ *Buf++; /* calculate the CRC  */
		crc_lo = crc_hi ^ table_crc_hi[i];
		crc_hi = table_crc_lo[i];
	}
	return (crc_hi << 8 | crc_lo);
}

//------------------------------------------------------------------------------
bool CheckCRC16(pbyte Data, word Size)
{
	uint16_t myCRC = CalcCRC(Data, Size - 2);
	uint16_t RecvdCRC = Data[Size - 1] << 8 | Data[Size - 2];
	return (myCRC == RecvdCRC);
}
//------------------------------------------------------------------------------
// Converts a Char to Hex nibble (4-bit value)
//------------------------------------------------------------------------------
byte nibble(char V)
{
	if (V >= '0' && V <= '9') return (V - '0');
	if (V >= 'A' && V <= 'F') return (V - 'A' + 0x0A);
	if (V >= 'a' && V <= 'f') return (V - 'a' + 0x0A);
	return 0; // should never happen in well format frame, we don't worry now about it
			  // surely this oddity will be detected during the PDU analysis
}
