...
//------------------------------------------------------------------------------
TMBSerClient::TMBSerClient(int Format, const char* PortName, int BaudRate, char Parity, int DataBits, int Stops, int Flow)
{
	...
	SetDeviceDefaults(Device);
	Device.SerialFormat = Format;
	memset(&DeviceStat, 0, sizeof(DeviceStat));
	ClearErrors();
}
//------------------------------------------------------------------------------
TMBSerClient::~TMBSerClient()
{
	ChannelsManager_DelChannel(sersck);
}
//------------------------------------------------------------------------------
PDeviceStatus TMBSerClient::GetDeviceStatusPtr(byte DeviceID)
{
	return PDeviceStatus(&DeviceStat);
}
//------------------------------------------------------------------------------
PDeviceParams TMBSerClient::GetDeviceParamsPtr(byte DeviceID)
{
	return PDeviceParams(&Device);
}
//------------------------------------------------------------------------------
void TMBSerClient::SetStatus(byte DeviceID, longword Time, int JobResult)
{
	DeviceStat.LastError = JobResult;
	DeviceStat.JobTime = Time;
}
//------------------------------------------------------------------------------
int TMBSerClient::GetDeviceStatus(int DeviceID, TDeviceStatus& DeviceStatus)
{
